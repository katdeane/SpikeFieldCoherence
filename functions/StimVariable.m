...
% other conditions in other pipelines, leaving if statement for easier
...
if matches(type,'Awake')
    ...
    elseif matches(Condition,'Spontaneous')
        stimList = 1; % cue for singlestim condition
        thisUnit = [];
    ...
    elseif contains(Condition,'gapASSR') 
        ...
        stimITI  = 500*sr_mult; % processing 500 ms but ITI actually 520 ms
        thisTag  = 'gapASSRRate';
    end
else
    error('add input type: Awake')
end
